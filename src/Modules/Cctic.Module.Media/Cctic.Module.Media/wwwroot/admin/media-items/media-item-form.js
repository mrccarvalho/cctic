...
(function ($) {
    angular
        ...
        .controller('MediaItemFormCtrl', MediaItemFormCtrl);

    /* @ngInject */
    function MediaItemFormCtrl($state, $stateParams, summerNoteService, mediaItemService, translateService) {
        var vm = this;
        ...
        function init() {
            if (vm.isEditMode) {
                newsItemService.getNewsItem(vm.newsItemId).then(function (result) {
                    ...
                    if (vm.newsItem.createdOn) {
                        vm.newsItem.createdOn = new Date(vm.newsItem.createdOn);
                    }
